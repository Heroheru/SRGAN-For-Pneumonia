...
class model_srgan(nn.Module):
    def __init__(self):
        super().__init__()
        self.generator = Generator()
        self.discriminator = Discriminator()

    def forward_generator(self, x_lr):
        return self.generator(x_lr)

    def forward_discriminator(self, x_hr):
        return self.discriminator(x_hr)

model = model_srgan()
from torchsummary import summary
device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
summary(model.generator.to(device), (3,96,96))  
